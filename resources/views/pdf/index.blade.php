...
<head>
    ...
    <style type="text/css">
		...
		ul.toc a::after {
			content: leader(dotted) target-counter(attr(href), attr(number));
		  	text-align: right;
			display: flex;
			/*margin-top: 5px;*/
		}
...
<body>
    <div class="contents">
        ...
        <ul class="toc">
        	@foreach($tableOfContent as $k => $content)
          		<li>
          			<a href="#{{ $content['id'] }}" title="{{ $content['title'] }}" number="{{ $content['page_number'] }}">
          				{{ $content['title'] }}
          			</a>
          		</li>
			@endforeach
